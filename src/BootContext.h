...
long encBatcherOddEvenSortRec(Ciphertext& sortedCipher, const Ciphertext& inCipher, long logNum, long logJump, long loc, Parameter parameter, long iter, double** mask, Scheme& scheme, BootHelper& boothelper) {
    ...
    if (logNum == 1) {
        ...
        // CompAndSwap()
        fcnEncCompAndSwapWithBoot(a, a, mask[loc], 1 << logJump, parameter, iter, scheme, boothelper);
        // compAndSwap(loc, 1 << logJump);
        // mask[loc] = genMaskingComp(1 << log2n, 1 << logJump);
    } else {
        if (logJump == 0) {
            loc = encBatcherOddEvenSortRec(a, a, logNum - 1, logJump, loc, parameter, iter, mask, scheme, boothelper);
            // loc = BatcherOddEvenSortRec(logNum - 1, logJump, loc);
        }
        loc = encBatcherOddEvenSortRec(a, a, logNum - 1, logJump + 1, loc, parameter, iter, mask, scheme, boothelper);
...
long encBatcherOddEvenSortRecWithDecrypt(Ciphertext& sortedCipher, const Ciphertext& inCipher, long logNum, long logJump, long loc, Parameter parameter, long iter, double** mask, Scheme& scheme, BootHelper& boothelper, SecretKey& secretKey) {
    ...
    if (logNum == 1) {
        // cout << "mask[" << loc << "] <- S(" << logNum << ", " << logJump << ")" << endl;
        // CompAndSwap()
        fcnEncCompAndSwapWithBootAndDecrypt(a, a, mask[loc], 1 << logJump, parameter, iter, scheme, boothelper, secretKey);
        // compAndSwap(loc, 1 << logJump);
        // mask[loc] = genMaskingComp(1 << log2n, 1 << logJump);
    } else {
        if (logJump == 0) {
            loc = encBatcherOddEvenSortRecWithDecrypt(a, a, logNum - 1, logJump, loc, parameter, iter, mask, scheme, boothelper, secretKey);
            // loc = BatcherOddEvenSortRec(logNum - 1, logJump, loc);
        }
        loc = encBatcherOddEvenSortRecWithDecrypt(a, a, logNum - 1, logJump + 1, loc, parameter, iter, mask, scheme, boothelper, secretKey);
        // loc = BatcherOddEvenSortRec(logNum - 1, logJump + 1, loc);
        // cout << "mask[" << loc << "] <- S(" << logNum << ", " << logJump << ")" << endl;
        // mask[loc] = genMaskingMerge(1 << log2n, 1 << logNum, 1 << logJump);
        fcnEncCompAndSwapWithBootAndDecrypt(a, a, mask[loc], 1 << logJump, parameter, iter, scheme, boothelper, secretKey);
...
void fcnEncCompAndSwapWithBootAndDecrypt(Ciphertext& outCipher, const Ciphertext& inCipher, double* mask, long dist, Parameter parameter, long iter, Scheme& scheme, BootHelper& boothelper, SecretKey& secretKey) {
    long logp = parameter.logp;
    Ciphertext a = Ciphertext(inCipher);
    long n = a.n;
